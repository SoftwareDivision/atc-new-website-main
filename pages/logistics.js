  ...
  return (
    <section className="container mx-auto px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pb-20">
        {logistics.map((post, index) => (
          <Link href={`/logistics/${post.logisticsSlug}`} key={index} className="no-underline">
            <div className="w-[400px] h-[480px] bg-white shadow-lg rounded-lg p-5 flex flex-col transition-transform hover:scale-105 mb-8">
              ...
              <div className="h-[220px] w-full overflow-hidden">
                <img
                  ...
                  src={post.logisticsMatter.coverImage}
                  alt={post.logisticsMatter.title}
                />
              </div>
...
export async function getStaticProps() {
  ...
  const logisticsPosts = files.map((filename) => {
    const logisticsSlug = filename.replace(".md", "");
    const markdownWithMeta = fs.readFileSync(
      path.join("logistics", filename),
      "utf-8"
    );

    const { data: logisticsMatter } = matter(markdownWithMeta);
  ...
  const sortedPosts = desiredOrder
    .map(slug => logisticsPosts.find(post => post.logisticsSlug === slug))
    .filter(Boolean); // This removes any undefined items in case a file is missing


  return {
    props: {
      logistics: sortedPosts,
    },
  };
}
