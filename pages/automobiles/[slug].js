...
function automobilesPage({
  automobilesMatter: { title, date, coverImage, author },
  slug,
  content,
}) {
  return (
...
export async function getStaticProps({ params: { slug } }) {
  const markdownwithMeta = fs.readFileSync(
    path.join("automobiles", slug + ".md"),
    "utf-8"
  );

  const { data: automobilesMatter, content } = matter(markdownwithMeta);
