...
function aluminiumPage({
  aluminiumMatter: { title, date, coverImage, author },
  slug,
  content,
}) {
  return (
...
export async function getStaticProps({ params: { slug } }) {
  const markdownwithMeta = fs.readFileSync(
    path.join("aluminium", slug + ".md"),
    "utf-8"
  );

  const { data: aluminiumMatter, content } = matter(markdownwithMeta);
