...
function retailsndistributionPage({
  retailsndistributionMatter: { title, date, coverImage, author },
  slug,
  content,
}) {
  return (
...
export default retailsndistributionPage;
...
export async function getStaticProps({ params: { slug } }) {
  const markdownwithMeta = fs.readFileSync(
    path.join("retailsndistribution", slug + ".md"),
    "utf-8"
  );

  const { data: retailsndistributionMatter, content } = matter(markdownwithMeta);
